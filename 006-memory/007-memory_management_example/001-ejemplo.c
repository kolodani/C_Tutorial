...
struct list
{
    ...
    int size;     // Indicates how many items fit in the allocted memory
};
...
int main()
{
    ...
    // Find out if memory allocarion was successful
    if (myList.data == NULL)
    {
...
void addToList(struct list *myList, int item)
{
    // If the list is full the resize the memory to fit 10 more items
